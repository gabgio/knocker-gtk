...
char *knocker_core_get_hostname (knocker_core_portscan_data_t *data)
{
   ...
   if (knocker_core_last_hostname == NULL)
     {
        knocker_core_last_hostname =
	        malloc( 1 + strlen(knocker_core_get_host_name_string (&data->host)));
     }
   else
     {
        knocker_core_last_hostname =
	        realloc(knocker_core_last_hostname, 1 + strlen(knocker_core_get_host_name_string (&data->host)));
...
char *knocker_core_get_hostip (knocker_core_portscan_data_t *data)
{
   ...
   if (knocker_core_last_hostip == NULL)
     {
        knocker_core_last_hostip =
	        malloc(1 + strlen(knocker_core_get_host_ip_string (&data->host)));
     }
   else
     {
        knocker_core_last_hostip =
	        realloc(knocker_core_last_hostip, 1 + strlen(knocker_core_get_host_ip_string (&data->host)));
...
fprintf(stderr, "debug: returned service by knocker_core_getservbyport is \"%s\", (%d bytes)\n", tmpstr, strlen(tmpstr));
  ...
  if (slenght == -1)
    {
      if (knocker_core_last_service != NULL)
        {
           free(knocker_core_last_service);
	   knocker_core_last_service = NULL;
	}
  ...
  if (knocker_core_last_service == NULL)
    {
       knocker_core_last_service =
	        malloc(slenght + 1);
    }
   else
    {
        knocker_core_last_service =
	        realloc(knocker_core_last_service, slenght + 1);
