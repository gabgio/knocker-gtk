...
static void print_help_info (void)
{
  ...
  fprintf (stdout, "   or: %s %s <HOST> %s <PORT>\n", PACKAGE, HOST_LONG_OPT, SINGLE_PORT_LONG_OPT);
  fprintf (stdout, "   or: %s %s <HOST> %s <PORT> %s <PORT> %s\n", PACKAGE, HOST_SHORT_OPT, START_PORT_SHORT_OPT, END_PORT_SHORT_OPT, QUIET_MODE_LONG_OPT);
  fprintf (stdout, "\n");
